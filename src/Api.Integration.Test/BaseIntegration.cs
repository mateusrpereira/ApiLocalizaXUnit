...
namespace Api.Integration.Test
{
    public abstract class BaseIntegration : IDisposable
    {
        ...
        {
            hostApi = "http://localhost:7224/api/";

            var builder = new WebHostBuilder().UseEnvironment("Testing").UseStartup<Startup>();

            var server = new TestServer(builder);//PROBLEMA AQUI    
        ...
        public async Task AdicionarToken()
        {
            ...
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", loginObject.accessToken);//tinha + 
        }
